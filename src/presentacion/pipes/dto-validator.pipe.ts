...
import { ResponseDTO, ValidationErrorsType } from 'src/shared/dto/response.dto';
...
export class DtoValidatorPipe implements PipeTransform<any> {
  async transform(value: any, metadata: ArgumentMetadata) {
    ...
    const metadataClass = Reflect.getMetadata(
      'dto-pipe-class-transform-options',
      metatype,
    );
...
export const formatValidationErrorsToResponseDto = <T>(
  ...
  const errorSms: Partial<ValidationErrorsType<T>> = errors.reduce(
    (objErrors, err) => {
  ...
  return {
    error: true,
    message:
      'hay un error de validación, por favor verifique los datos ingresados',
    response: {
